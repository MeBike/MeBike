# Test Cases cho Hàm: `resetPassword`

**Nằm trong file**: auth.controller.ts
**Function Code**: FUNC_RESETPASSWORD
...
#### TC_002: Xử lý ngoại lệ `InvalidResetToken` (Abnormal Flow)
**a. Condition:**
- **Precondition**: Gặp trường hợp ngoại lệ liên quan đến `InvalidResetToken` trong quá trình xử lý.
- **Values of inputs (Abnormal)**: Thông số đầu vào không hợp lệ với yêu cầu của trạng thái hiện tại (hoặc đối tượng không tồn tại).
**b. Confirmation:**
- **Output result**: Lỗi 400/403/404, hiển thị mã error: `InvalidResetToken`.
**c. Type & Result:**
- **Type**: Abnormal
- **Result**: [ ] P / F
