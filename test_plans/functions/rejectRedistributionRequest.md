# Test Cases cho Hàm: `rejectRedistributionRequest`

**Nằm trong file**: manager.controller.ts
**Function Code**: FUNC_REJECTREDISTRIBUTIONREQUEST
**Function Name**: `rejectRedistributionRequest`
...
#### TC_004: Xử lý ngoại lệ `UnauthorizedRedistributionRejection` (Abnormal Flow)
**a. Condition:**
- **Precondition**: Gặp trường hợp ngoại lệ liên quan đến `UnauthorizedRedistributionRejection` trong quá trình xử lý.
- **Values of inputs (Abnormal)**: Thông số đầu vào không hợp lệ với yêu cầu của trạng thái hiện tại (hoặc đối tượng không tồn tại).
**b. Confirmation:**
- **Output result**: Lỗi 400/403/404, hiển thị mã error: `UnauthorizedRedistributionRejection`.
**c. Type & Result:**
- **Type**: Abnormal
- **Result**: [ ] P / F

#### TC_005: Xử lý ngoại lệ `CannotRejectNonPendingRedistribution` (Abnormal Flow)
**a. Condition:**
- **Precondition**: Gặp trường hợp ngoại lệ liên quan đến `CannotRejectNonPendingRedistribution` trong quá trình xử lý.
- **Values of inputs (Abnormal)**: Thông số đầu vào không hợp lệ với yêu cầu của trạng thái hiện tại (hoặc đối tượng không tồn tại).
**b. Confirmation:**
- **Output result**: Lỗi 400/403/404, hiển thị mã error: `CannotRejectNonPendingRedistribution`.
**c. Type & Result:**
- **Type**: Abnormal
- **Result**: [ ] P / F
