# Test Cases cho Hàm: `registerPushToken`

**Nằm trong file**: self.controller.ts
**Function Code**: FUNC_REGISTERPUSHTOKEN
**Function Name**: `registerPushToken`
...
#### TC_002: Xử lý ngoại lệ `InvalidPushToken` (Abnormal Flow)
**a. Condition:**
- **Precondition**: Gặp trường hợp ngoại lệ liên quan đến `InvalidPushToken` trong quá trình xử lý.
- **Values of inputs (Abnormal)**: Thông số đầu vào không hợp lệ với yêu cầu của trạng thái hiện tại (hoặc đối tượng không tồn tại).
**b. Confirmation:**
- **Output result**: Lỗi 400/403/404, hiển thị mã error: `InvalidPushToken`.
**c. Type & Result:**
- **Type**: Abnormal
- **Result**: [ ] P / F
