# Test Cases cho Hàm: `createStripeTopupPaymentSheet`

**Nằm trong file**: me.controller.ts
**Function Code**: FUNC_CREATESTRIPETOPUPPAYMENTSHEET
**Function Name**: `createStripeTopupPaymentSheet`
...
#### TC_002: Xử lý ngoại lệ `InvalidTopupRequest` (Abnormal Flow)
**a. Condition:**
- **Precondition**: Gặp trường hợp ngoại lệ liên quan đến `InvalidTopupRequest` trong quá trình xử lý.
- **Values of inputs (Abnormal)**: Thông số đầu vào không hợp lệ với yêu cầu của trạng thái hiện tại (hoặc đối tượng không tồn tại).
**b. Confirmation:**
- **Output result**: Lỗi 400/403/404, hiển thị mã error: `InvalidTopupRequest`.
**c. Type & Result:**
- **Type**: Abnormal
- **Result**: [ ] P / F
...
#### TC_004: Xử lý ngoại lệ `TopupProviderError` (Abnormal Flow)
**a. Condition:**
- **Precondition**: Gặp trường hợp ngoại lệ liên quan đến `TopupProviderError` trong quá trình xử lý.
- **Values of inputs (Abnormal)**: Thông số đầu vào không hợp lệ với yêu cầu của trạng thái hiện tại (hoặc đối tượng không tồn tại).
**b. Confirmation:**
- **Output result**: Lỗi 400/403/404, hiển thị mã error: `TopupProviderError`.
**c. Type & Result:**
- **Type**: Abnormal
- **Result**: [ ] P / F
