# Test Cases cho Hàm: `getMyRentalImpact`

**Nằm trong file**: impact.controller.ts
**Function Code**: FUNC_GETMYRENTALIMPACT
**Function Name**: `getMyRentalImpact`
...
#### TC_002: Xử lý ngoại lệ `EnvironmentImpactNotFound` (Abnormal Flow)
**a. Condition:**
- **Precondition**: Gặp trường hợp ngoại lệ liên quan đến `EnvironmentImpactNotFound` trong quá trình xử lý.
- **Values of inputs (Abnormal)**: Thông số đầu vào không hợp lệ với yêu cầu của trạng thái hiện tại (hoặc đối tượng không tồn tại).
**b. Confirmation:**
- **Output result**: Lỗi 400/403/404, hiển thị mã error: `EnvironmentImpactNotFound`.
**c. Type & Result:**
- **Type**: Abnormal
- **Result**: [ ] P / F
