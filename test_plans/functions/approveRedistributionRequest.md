# Test Cases cho Hàm: `approveRedistributionRequest`

**Nằm trong file**: manager.controller.ts
**Function Code**: FUNC_APPROVEREDISTRIBUTIONREQUEST
**Function Name**: `approveRedistributionRequest`
...
#### TC_004: Xử lý ngoại lệ `UnauthorizedRedistributionApproval` (Abnormal Flow)
**a. Condition:**
- **Precondition**: Gặp trường hợp ngoại lệ liên quan đến `UnauthorizedRedistributionApproval` trong quá trình xử lý.
- **Values of inputs (Abnormal)**: Thông số đầu vào không hợp lệ với yêu cầu của trạng thái hiện tại (hoặc đối tượng không tồn tại).
**b. Confirmation:**
- **Output result**: Lỗi 400/403/404, hiển thị mã error: `UnauthorizedRedistributionApproval`.
**c. Type & Result:**
- **Type**: Abnormal
- **Result**: [ ] P / F

#### TC_005: Xử lý ngoại lệ `CannotApproveNonPendingRedistribution` (Abnormal Flow)
**a. Condition:**
- **Precondition**: Gặp trường hợp ngoại lệ liên quan đến `CannotApproveNonPendingRedistribution` trong quá trình xử lý.
- **Values of inputs (Abnormal)**: Thông số đầu vào không hợp lệ với yêu cầu của trạng thái hiện tại (hoặc đối tượng không tồn tại).
**b. Confirmation:**
- **Output result**: Lỗi 400/403/404, hiển thị mã error: `CannotApproveNonPendingRedistribution`.
**c. Type & Result:**
- **Type**: Abnormal
- **Result**: [ ] P / F
