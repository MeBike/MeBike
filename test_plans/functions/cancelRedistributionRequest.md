# Test Cases cho Hàm: `cancelRedistributionRequest`

**Nằm trong file**: staff.controller.ts
**Function Code**: FUNC_CANCELREDISTRIBUTIONREQUEST
**Function Name**: `cancelRedistributionRequest`
...
#### TC_002: Xử lý ngoại lệ `CannotCancelNonPendingRedistribution` (Abnormal Flow)
**a. Condition:**
- **Precondition**: Gặp trường hợp ngoại lệ liên quan đến `CannotCancelNonPendingRedistribution` trong quá trình xử lý.
- **Values of inputs (Abnormal)**: Thông số đầu vào không hợp lệ với yêu cầu của trạng thái hiện tại (hoặc đối tượng không tồn tại).
**b. Confirmation:**
- **Output result**: Lỗi 400/403/404, hiển thị mã error: `CannotCancelNonPendingRedistribution`.
**c. Type & Result:**
- **Type**: Abnormal
- **Result**: [ ] P / F

#### TC_003: Xử lý ngoại lệ `UnauthorizedRedistributionCancellation` (Abnormal Flow)
**a. Condition:**
- **Precondition**: Gặp trường hợp ngoại lệ liên quan đến `UnauthorizedRedistributionCancellation` trong quá trình xử lý.
- **Values of inputs (Abnormal)**: Thông số đầu vào không hợp lệ với yêu cầu của trạng thái hiện tại (hoặc đối tượng không tồn tại).
**b. Confirmation:**
- **Output result**: Lỗi 400/403/404, hiển thị mã error: `UnauthorizedRedistributionCancellation`.
**c. Type & Result:**
- **Type**: Abnormal
- **Result**: [ ] P / F
