...
>
> `GET /v1/coupons` has been removed from the server contract. V1 no longer
> supports "my coupons"; users do not own individual coupons in `user_coupons`.
> Wallet rentals without `subscription_id` get the best eligible global discount
> from `coupon_rules` automatically. Use
