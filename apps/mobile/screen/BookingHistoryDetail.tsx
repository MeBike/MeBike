...
function BookingHistoryDetail() {
  ...
  const { data: existingRating } = useGetRatingQuery(bookingId, Boolean(rentalResult));
            ...
            {hasRated && existingRating
              ? (
                  <View style={styles.existingRatingContainer}>
                    ...
                    {existingRating.reason_details && existingRating.reason_details.length > 0 && (
                      <>
                        <Text style={styles.reasonsTitle}>Lý do đánh giá:</Text>
                        <View style={styles.selectedReasonsContainer}>
                          {existingRating.reason_details.map(reason => (
