...
import { getBikeStatusLabel } from "@utils/bike";
  ...
  return (
    <View style={styles.card}>
      <Text
        ...
        numberOfLines={1}
      >
        {`Xe #${shortId(bike.id, { head: 8, tail: 4 })}`}
      </Text>
      ...
      <View style={{ marginTop: 16, gap: 10 }}>
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoValue} numberOfLines={1} ellipsizeMode="middle">
            {bike.supplierId ? shortId(bike.supplierId) : "Chưa cập nhật"}
          </Text>
        </View>
        {Boolean(bike.createdAt) && (
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>Ngày tạo</Text>
            <Text style={styles.infoValue} numberOfLines={1}>
              {formatVietnamDateTime(bike.createdAt)}
            </Text>
          </View>
        )}
      </View>
