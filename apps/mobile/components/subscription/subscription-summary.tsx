...
export function SubscriptionSummary({
  ...
  const title = hasActive
    ? `Gói ${activeSubscription?.package_name ?? ""}`.trim()
    : hasPending
    ...
    if (hasActive && activeSubscription) {
      ...
      const quota
        = typeof limit === "number"
          ...
          : `${used} lượt đã dùng`;
      return `Hạn: ${formatVietnamDateTime(activeSubscription.expires_at ?? "")}
${quota}`;
    }
    if (hasPending && pendingSubscription) {
      return `Đăng ký ngày ${formatVietnamDateTime(pendingSubscription.created_at ?? "")} - kích hoạt để sử dụng ngay.`;
    }
