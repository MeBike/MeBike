...
export const walletService = {
  ...
  topUpWallet: async (
    ...
    const response = await fetchHttpClient.put<MyWallet>(WALLET_ENDPOINTS.TOP_UP, data);
    return response;
  },
  debitWallet: async (
    data: DecreaseSchemaFormData
  ): Promise<AxiosResponse<MyWallet>> => {
    const response = await fetchHttpClient.put<MyWallet>(WALLET_ENDPOINTS.DEBIT, data);
    return response;
  },
};