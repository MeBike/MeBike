    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-500">
      <div className="bg-gradient-to-br from-card via-card to-muted/20 border border-border rounded-2xl p-8 shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <SummaryCard title="Trạm" value={result.summary.totalStations} color="blue" />
            <SummaryCard title="Doanh Thu" value={result.summary.totalRevenueFormatted} color="green" isGradient />
            <SummaryCard title="Lượt Thuê" value={result.summary.totalRentals} color="purple" />
            <SummaryCard title="Trung Bình" value={result.summary.avgRevenuePerStationFormatted} color="orange" />
        </div>
...
function StationDetailCard({ station, index }: { station: StationStatistic, index: number }) {
    return (
        <div className="group bg-card border border-border rounded-xl p-6 hover:border-primary/50 transition-all shadow-sm" style={{ animationDelay: `${index * 50}ms` }}>
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h4 className="text-lg font-semibold group-hover:text-primary transition-colors">{station.name}</h4>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                    <div className="text-center px-4 py-2 bg-muted/50 rounded-lg">
                        ...
                        <p className="font-bold text-green-600">{station.totalRevenueFormatted}</p>
                    </div>
                    <div className="text-center px-4 py-2 bg-muted/50 rounded-lg">
                        <p className="text-[10px] uppercase text-muted-foreground">Lượt thuê</p>
                        <p className="font-bold text-blue-600">{station.totalRentals}</p>
                    </div>
                    <div className="text-center px-4 py-2 bg-muted/50 rounded-lg col-span-2 lg:col-span-1">
                        ...
                        <p className="font-bold text-purple-600">{station.totalDurationFormatted}</p>
                    </div>
                </div>
            </div>
        </div>
