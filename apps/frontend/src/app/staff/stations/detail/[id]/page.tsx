...
function StatusItem({
  ...
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-border/40 last:border-0 last:pb-0">
      ...
      <span className={cn("text-sm", boldValue ? "font-bold" : "font-semibold", color)}>{value || 0}</span>
    </div>
  );
}
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {station.id !== listStation?.currentStation?.id && (
              <Button
                onClick={() =>
                  router.push(`/staff/distribution-request/create?targetStationId=${station.id}`)
                }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <MetricCard
            ...
            value={station.capacity.emptyPhysicalSlots}
              ...
              <div className="space-y-5">
                <div className="flex items-center justify-between p-4 rounded-xl bg-muted/40 border border-border/50">
                   <div className="space-y-1">
                      <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Chỗ trống thực tế</p>
                      <p className="text-3xl font-bold text-foreground">
                        {station.capacity.emptyPhysicalSlots ?? station.returnSlots.available} <span className="text-sm font-normal text-muted-foreground">/ {station.capacity.total}</span>
                      </p>
                   </div>
