...
export default function AdminRentalDetailClient({
  ...
  data,
  isLoading,
}: AdminRentalDetailClientProps) {
  const router = useRouter();

  // Khung Skeleton hiển thị khi đang tải
  const loadingShell = (
          ...
          <div className="space-y-6 lg:col-span-2">
            ...
            <SectionCard icon={Bike} title="Phương tiện">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <Field
                  label="Mã Chip"
                  value={data.bike?.chipId || "Chưa có dữ liệu"}
                />
                <Field
                  label="Nhà cung cấp"
                  value={data.bike?.supplierId || "Chưa có dữ liệu"}
                />
              </div>
            </SectionCard>
          </div>
