...
const STATUS_CONFIG_AGENCY_REQUEST: Record<
  string,
    ...
    <div className="mx-auto max-w-7xl space-y-6 animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className="flex items-center gap-1 text-xs text-muted-foreground">
              <Calendar className="h-3 w-3" /> Gửi ngày:{" "}
              {data.createdAt ? formatToVNTime(data.createdAt) : "N/A"}
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        <DetailSection icon={User} title="Người yêu cầu">
          <DetailRow
            label="Họ tên"
            value={data.requesterUser?.fullName}
            icon={User}
          />
          <DetailRow
            label="Email liên hệ"
        ...
        <DetailSection icon={Building2} title="Thông tin Agency">
          ...
          />
          <DetailRow
            label="SĐT kinh doanh"
            value={data.agencyContactPhone}
            icon={Phone}
          />
        </DetailSection>
            ...
            {data.status !== "PENDING" && (
              <div className="rounded-lg border border-primary/10 bg-primary/5 p-3">
                ...
                <p className="text-sm font-medium">
                  {data.reviewedByUser?.fullName || "Hệ thống"}
                </p>
              </div>
            )}
          </div>
