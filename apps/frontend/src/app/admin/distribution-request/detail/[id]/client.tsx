      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-3">
            ...
            <Badge
              className={`${getStatusStyle(data.status)} px-3 py-1 text-xs font-bold uppercase shadow-sm`}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card className="shadow-sm border-slate-200">
              <CardHeader className="pb-3 border-b bg-slate-50/50">
                <CardTitle className="text-sm font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
                  <ClipboardList className="h-4 w-4 text-blue-500" /> Người yêu
                  cầu
                </CardTitle>
              </CardHeader>
            ...
            <Card
              className={`shadow-sm border-slate-200 ${!data.approvedByUser ? "bg-slate-50/40 border-dashed" : ""}`}
            >
              <CardHeader className="pb-3 border-b bg-slate-50/50">
                <CardTitle className="text-sm font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
                  <ActionIcon
                    className={`h-4 w-4 ${isRejected || isCancelled ? "text-red-500" : "text-green-500"}`}
                  />
                  {actionUserLabel}
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-4 space-y-3">
                {data.approvedByUser ? (
                  <>
                    <div className="flex items-center gap-3">
                      <div
                        className={`h-10 w-10 rounded-full flex items-center justify-center font-bold shrink-0 ${isRejected || isCancelled ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
                      >
                        {data.approvedByUser.fullName.charAt(0)}
                      </div>
                      <div className="min-w-0">
                        <p className="font-bold text-slate-900 truncate">
                          {data.approvedByUser.fullName}
                        </p>
                        <p className="text-xs text-muted-foreground truncate">
                          {data.approvedByUser.email}
                        </p>
                      </div>
                    </div>
                    <div className="pt-2 border-t">
          ...
          <Card className="shadow-lg border-none overflow-hidden">
            <CardHeader className="bg-slate-900 text-white py-4">
              <CardTitle className="text-md font-medium flex items-center gap-2">
                <Bike className="h-5 w-5 text-blue-400" /> Danh sách xe thực tế
                ({data.items.length})
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              <Table>
                <TableHeader className="bg-slate-50">
                  <TableRow>
                    <TableHead className="w-[80px] text-center font-bold text-xs uppercase">
                      STT
                    </TableHead>
                      ...
                      <TableRow key={item.id} className="hover:bg-blue-50/30">
                        ...
                        <TableCell>
                          <div className="flex flex-col gap-0.5">
                            ...
                            <code className="text-[9px] text-blue-600 bg-blue-50 w-fit px-1 rounded font-mono">
                              {item.bike.id}
                            </code>
                          </div>
                        </TableCell>
        ...
        <div className="space-y-6">
                    ...
                    {data.completedAt && (
                      <div>
                        ...
                        <p className="text-[10px] font-bold text-green-600 uppercase">
                          {REQUEST_STATUS_VI.COMPLETED}
                        </p>
                        <p className="text-sm font-semibold">
                          {formatToVNTime(data.completedAt)}
                        </p>
                      </div>
                    )}
                  </>
          ...
          <Card
            className={`shadow-sm border-dashed border-2 ${isRejected || isCancelled ? "bg-red-50/50 border-red-200" : "bg-amber-50/50 border-amber-200"}`}
          >
            <CardHeader className="pb-2">
              <CardTitle
                className={`text-xs font-bold uppercase flex items-center gap-2 ${isRejected || isCancelled ? "text-red-700" : "text-amber-700"}`}
              >
            ...
            <CardContent>
              <p
                className={`text-sm italic ${isRejected || isCancelled ? "text-red-900" : "text-amber-900"}`}
              >
