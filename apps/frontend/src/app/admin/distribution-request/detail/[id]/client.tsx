...
// --- CONFIGS & HELPERS ---
export const STATUS_MAP: Record<string, { label: string; style: string; icon: any }> = {
    ...
    return (
      <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100 transition-colors hover:bg-slate-100">
        ...
        <div className="flex flex-col overflow-hidden">
          ...
          <span className="text-xs text-slate-500 truncate">{user?.email || "N/A"}</span>
        </div>
      </div>
    );
  };
        ...
        <div className="flex flex-col lg:flex-row">
                  ...
                  <div className="space-y-3 ml-2">
                    <div className="flex justify-between items-center text-sm border-b border-slate-50 pb-2">
                      ...
                      <span className="font-semibold text-slate-700">
                        {(data.sourceStation as any)?.availableBikesBefore ?? data.sourceAvailableBikesBefore ?? 0} xe
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-sm border-b border-slate-50 pb-2">
                      <span className="text-slate-500">Số xe xuất đi</span>
                      <span className="font-bold text-orange-600">
                        -{(data.sourceStation as any)?.bikesForRedistribution ?? data.requestedQuantity ?? 0} xe
                      </span>
                    </div>
                ...
                <div className="border border-slate-100 rounded-xl p-5 shadow-sm relative overflow-hidden bg-white">
                  ...
                  <div className="space-y-3 ml-2">
                    <div className="flex justify-between items-center text-sm border-b border-slate-50 pb-2">
                      <span className="text-slate-500">Trước điều phối</span>
                      <span className="font-semibold text-slate-700">
                        {(data.targetStation as any)?.availableBikesBefore ?? data.targetAvailableBikesBefore ?? 0} xe
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-sm border-b border-slate-50 pb-2">
                      ...
                      <div className="text-right">
                        <span className={`font-bold ${isReverted && successfulBikes === 0 ? 'text-slate-400' : 'text-blue-600'}`}>
                          {(data.targetStation as any)?.actualReceivedBikes ?? successfulBikes}
                        </span>
                    ...
                    <div className="flex justify-between items-center text-sm pt-1">
                      ...
                      <span className="font-bold text-slate-900 text-base">
                        {(data.targetStation as any)?.actualAvailableBikes ?? 
                         (((data.targetStation as any)?.availableBikesBefore ?? data.targetAvailableBikesBefore ?? 0) + 
                          ((data.targetStation as any)?.actualReceivedBikes ?? successfulBikes))} xe
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <div className="w-full lg:w-[400px] bg-slate-50 p-6 lg:p-8 border-t lg:border-t-0 lg:border-l border-slate-200 flex flex-col gap-6">
            ...
            <div className="space-y-3">
              <div className={`bg-gradient-to-br ${getHeroColor()} rounded-2xl p-6 text-white shadow-lg relative overflow-hidden transition-colors`}>
                <div className="relative z-10">
                  <span className="text-white/80 text-xs font-bold uppercase tracking-wider mb-1 block">Yêu cầu điều phối</span>
            ...
            <div className="pt-4 border-t border-slate-200 space-y-3">
              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Nhân sự liên quan</h4>
              ...
              {(data as any).rejectedByUser && data.status === "REJECTED" && (
                <UserProfileMini user={(data as any).rejectedByUser} label="Người từ chối" />
              )}
              {data.revertedByUser && (data.revertedBikes > 0 || isReverted) && (
                <UserProfileMini user={data.revertedByUser} label="Người báo hoàn trả" />
              )}
            </div>
          </div>
        </div>
