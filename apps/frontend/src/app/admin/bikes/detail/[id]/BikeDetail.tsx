    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 rounded-lg border border-border/60 bg-muted/40 px-4 py-3 text-sm sm:flex-row sm:items-center sm:gap-x-8">
        ...
        <div>
          ...
          <span className="font-semibold text-foreground">{bike.bikeNumber || "N/A"}</span>
        </div>
        <div>
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <SectionCard icon={BikeIcon} title="Thông tin cơ bản">
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              />
              <Field label="Nhà cung cấp" value={bike.supplier?.name || "Hệ thống"} />
              
              {/* Thêm hiển thị Ratings vào đây */}
              <div>
                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Đánh giá khách hàng</p>
                <div className="mt-1 flex items-center gap-2">
                  <div className="flex items-center gap-1 text-amber-500">
                    <Star className="h-4 w-4 fill-current" />
                    <span className="text-sm font-bold text-foreground">
                      {bike.rating?.averageRating || bike.averageRating || "0.0"}
                    </span>
                  </div>
                  <span className="text-xs text-muted-foreground">
                    ({bike.rating?.totalRatings || bike.totalRatings || 0} lượt)
                  </span>
                </div>
              </div>
            </div>
          ...
          <SectionCard icon={Activity} title="Thống kê">
            <div className="space-y-4">
              <div className="flex justify-between items-center border-b pb-2">
                ...
                <span className="font-bold">{statisticData?.totalRentals || 0}</span>
              </div>
              <div className="flex justify-between items-center border-b pb-2">
                <span className="text-sm text-muted-foreground">Hoạt động</span>
                <span className="font-bold">{totalHours}h {activity ? activity.totalMinutesActive % 60 : 0}m</span>
              </div>
              <div className="flex justify-between items-center border-b pb-2">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-border/40">
              {rentals.length > 0 ? (
                rentals.map((rental) => (
                  <tr key={rental.id} className="group hover:bg-muted/30 transition-colors">
                    ...
                    <td className="py-4 pr-4 text-xs">
                      <div className="flex items-center gap-1.5 font-medium">
                        {rental.startStation.name} <ArrowRight className="h-3 w-3 text-muted-foreground" /> {rental.endStation.name}
                      </div>
                    </td>
                ...
                <tr>
                  <td colSpan={4} className="py-8 text-center text-muted-foreground">Chưa có dữ liệu lịch sử.</td>
                </tr>
              )}
            </tbody>
          </table>
