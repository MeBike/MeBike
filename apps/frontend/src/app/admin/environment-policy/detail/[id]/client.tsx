    ...
    <div className="mx-auto max-w-6xl space-y-6 animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span
                className={`inline-flex px-1 py-1 rounded-full text-xxs font-medium ${STATUS_CONFIG[data.status].color}`}
              >
                {STATUS_CONFIG[data.status].label}
              </span>
              <span className="text-xs text-muted-foreground flex items-center gap-1">
                <Calendar className="h-3 w-3" /> Tạo lúc:{" "}
                {formatToVNTime(data.created_at)}
              </span>
            </div>
          </div>
        </div>
        {data.status === "INACTIVE" && (
          ...
          >
            {isProcessing ? (
              ...
              <>
                <CheckCircle2 className="mr-2 h-4 w-4" /> Kích hoạt chính sách
              </>
            )}
          </Button>
        )}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <DetailSection icon={Activity} title="Thông số Vận hành">
          <DetailRow
            ...
            value={data.average_speed_kmh}
            unit="km/h"
          />
          <DetailRow
            label="CO2 tiết kiệm / km"
            value={data.co2_saved_per_km}
            unit={data.co2_saved_per_km_unit}
          />
        </DetailSection>
        ...
        <DetailSection icon={Settings2} title="Cấu hình Công thức">
          <DetailRow
            ...
            value={data.formula_config.formula_version}
          />
          <DetailRow
            label="Nguồn dữ liệu"
            value={data.formula_config.distance_source}
          />
        </DetailSection>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <DetailSection icon={ShieldCheck} title="Tham số Bảo mật & Buffer">
          <DetailRow
            label="Hệ số tin cậy"
            value={data.formula_config.confidence_factor}
          />
          <DetailRow
            label="Thời gian Scan Buffer"
            value={data.formula_config.return_scan_buffer_minutes}
            unit="phút"
          />
        </DetailSection>
        ...
        <DetailSection icon={Leaf} title="Trạng thái Thời gian">
          <DetailRow
            ...
            value={
              data.active_from
                ? formatToVNTime(data.active_from)
                : "Chưa xác định"
            }
          />
          <DetailRow
            label="Ngày kết thúc"
            value={data.active_to ? formatToVNTime(data.active_to) : "Vô hạn"}
          />
        </DetailSection>
      </div>
    </div>
