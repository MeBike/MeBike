...
export function toWalletWithdrawalDetail(
  ...
  return {
    ...
    walletId: row.walletId,
    amount: row.amount.toString(),
    currency: row.currency as WalletsContracts.WalletWithdrawalDetail["currency"],
    payoutAmount: row.payoutAmount?.toString() ?? null,
    payoutCurrency: row.payoutCurrency as WalletsContracts.WalletWithdrawalDetail["payoutCurrency"],
    fxRate: row.fxRate?.toString() ?? null,
    fxQuotedAt: row.fxQuotedAt?.toISOString() ?? null,
    status: row.status,
