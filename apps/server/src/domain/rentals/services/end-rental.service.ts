...
export function endRentalByAdminUseCase(
  input: Omit<EndRentalInput, "userId">,
): Effect.Effect<
  RentalRow,
  RentalServiceFailure,
  | Prisma
...
> {
  return Effect.gen(function* () {
    ...
    yield* BikeRepository;
    yield* SubscriptionRepository;
    const { rentalId, endStationId, endTime } = input;

    const currentOpt = yield* repo.findById(rentalId).pipe(
      Effect.catchTag("RentalRepositoryError", error => Effect.die(error)),
    );

    if (Option.isNone(currentOpt)) {
      return yield* Effect.fail(new RentalNotFound({ rentalId, userId: "unknown" }));
    }
