      ...
      return Effect.gen(function* () {
        ...
        return {
          ...
          currentStation: {
            ...
            fixedBikes: bikeCounts.get(BikeStatus.FIXED) ?? 0,
            reservedBikes: bikeCounts.get(BikeStatus.RESERVED) ?? 0,
            pendingDispatchBikes: bikeCounts.get(BikeStatus.PENDING_DISPATCH) ?? 0,
            transportingBikes: bikeCounts.get(BikeStatus.TRANSPORTING) ?? 0,
            swappingBikes: bikeCounts.get(BikeStatus.SWAPPING) ?? 0,
            lostBikes: bikeCounts.get(BikeStatus.LOST) ?? 0,
            disabledBikes: bikeCounts.get(BikeStatus.DISABLED) ?? 0,
            emptySlots,
            occupancyRate,
          },
          pickups: {
