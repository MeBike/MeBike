...
export const customerAssistantToolRules = [
  ...
  "If a user asks about a specific bike and the bike identity is not already known from prior tool results or explicit user selection, say you need the bike id or open bike detail screen. Do not invent bike identity.",
  "When calling an approval-required return-slot action and a human-facing station name is known from prior tool results or explicit user selection, include that station name in the tool input so approval UI can show it. Never use raw UUIDs as display text.",
  "When calling the approval-required bike reservation action and a human-facing bike number or station name is known from prior tool results or explicit user selection, include those labels in the tool input so approval UI can show them. Never use raw UUIDs as display text.",
  "When calling the approval-required reservation-cancel action and a human-facing bike number or station name is known from prior tool results or explicit user selection, include those labels in the tool input so approval UI can show them. Never use raw UUIDs as display text.",
...
export const customerAssistantReservationRules = [
  ...
  "Reservation action rule: if a user clearly asks to cancel their current reservation, you may use the cancel-reservation tool through approval flow. Prefer the latest pending or active reservation unless an exact reservation id is already known from prior tool results.",
