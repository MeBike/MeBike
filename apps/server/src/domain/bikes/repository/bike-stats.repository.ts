    ...
    getRentalStats: () =>
      withOp("stats.rentalStats", async () => {
        const row = await db
          .selectFrom("Bike")
          .select([
            sql<number>`count(*) filter (where status not in ('DISABLED', 'LOST'))`.as(
              "total_active",
            ),
      ...
      withOp("stats.bikeStatistics", async () => {
        const row = await db
          ...
          .select([
            ...
            sql<number>`count(*) filter (where status = 'PENDING_DISPATCH')`.as("pending_dispatch"),
            sql<number>`count(*) filter (where status = 'TRANSPORTING')`.as("transporting"),
            sql<number>`count(*) filter (where status = 'SWAPPING')`.as("swapping"),
            sql<number>`count(*) filter (where status = 'FIXED')`.as("fixed"),
        ...
        return {
          ...
          TRANSPORTING: Number(row?.transporting ?? 0),
          SWAPPING: Number(row?.swapping ?? 0),
