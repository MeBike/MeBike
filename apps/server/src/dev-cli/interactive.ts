...
async function inspectStation(connectionString: string, value: string) {
  while (true) {
    ...
    writeLine(`${chalk.gray("status")}: available ${station.availableBikes}, booked ${station.bookedBikes}, reserved ${station.reservedBikes}, broken ${station.brokenBikes}, pendingDispatch ${station.pendingDispatchBikes}, transporting ${station.transportingBikes}, swapping ${station.swappingBikes}, lost ${station.lostBikes}, disabled ${station.disabledBikes}`);
